var jsPsychImageRangeSliderResponse = (function (jspsych) {
  'use strict';

  const info = {
      name: "image-range-slider-response",
      parameters: {
          ...
          /** Sets the starting values of the slider */
          slider_start: {
              type: jspsych.ParameterType.ARRAY,
              pretty_name: "Slider starting values",
              default: [25,75]
          },
  ...
  /**
   * **image-range-slider-response**
   *
   * jsPsych plugin for showing an image stimulus and getting a range slider response
   *
   * @author Kianté
   * @see {@link https://www.jspsych.org/plugins/jspsych-image-slider-response/ image-range-slider-response plugin is an extension to image-slider-response }
   */
  class ImageRangeSliderResponsePlugin {
      constructor(jsPsych) {
      ...
      trial(display_element, trial) {
          ...
          if (trial.render_on_canvas) {
             ...
             // create html string with two sliders and labels, and add to slider container
             //  html =
             //     '<input type="range" class="jspsych-slider" value="' +
             //     trial.slider_start[0] +
             //     '" min="' +
             //     trial.min +
             //     '" max="' +
             //     trial.max +
             //     '" step="' +
             //     trial.step +
             //     '" id="jspsych-image-slider-response-response-lower"></input>';
             //  html +=
             //     '<input type="range" class="jspsych-slider" value="' +
             //     trial.slider_start[1] +
             //     '" min="' +
             //     trial.min +
             //     '" max="' +
             //     trial.max +
             //     '" step="' +
             //     trial.step +
             //     '" id="jspsych-image-slider-response-response-upper"></input>';
              html = '<div id="jspsych-image-slider-response-response" class="jspsych-slider"></div>';
              ...
              display_element.appendChild(slider_container); // Ensure the slider container is added to the DOM
              var slider = document.getElementById('jspsych-image-slider-response-response');

              if (slider !== null) { // Check if the slider element exists
                noUiSlider.create(slider, {
                    start: trial.slider_start, // this should be an array of two numbers
                    step: trial.step,
                    range: {
                        'min': trial.min,
                        'max': trial.max
          ...
          const end_trial = () => {
              ...
              var trialdata = {
                  ...
                  slider_start_low: trial.slider_start[0],
                  slider_start_high: trial.slider_start[1],
...
//             .querySelector("#jspsych-image-slider-response-response") //this is the line I changed to make it move forward auto
              .querySelector("#jspsych-image-slider-response-next") //this is the line I changed to make it move forward auto
              .addEventListener("click", () => {
              // measure response time
              var endTime = performance.now();
              response.rt = Math.round(endTime - startTime);
              var sliderValues = slider.noUiSlider.get();
              response.response = sliderValues.map(Number);
            //  response.response = display_element.querySelector('#jspsych-image-slider-response-response').value;
            //   response.response = [
            //     document.querySelector("#jspsych-image-slider-response-response-lower").valueAsNumber,
            //     document.querySelector("#jspsych-image-slider-response-response-upper").valueAsNumber
            //   ];
//              response.response = display_element.querySelector("#jspsych-image-slider-response-response").valueAsNumber;
  ...
  ImageRangeSliderResponsePlugin.info = info;

  return ImageRangeSliderResponsePlugin;
