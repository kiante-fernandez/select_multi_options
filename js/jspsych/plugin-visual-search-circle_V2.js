var jsPsychVisualSearchCircle = (function (jspsych) {
  ...
  class VisualSearchCirclePlugin {
      ...
      trial(display_element, trial) {
        ...
        // Variables for tracking outbound and inbound RTs
        var lastEventTime; // Time of search array appearance or last fixation click
        var imageClickTime; // Time of the last image click
        var selections = []; // Array to store selection information with outbound and inbound RTs
        ...
        const imageClickHandler = (event) => {
            ...
            var outboundRT = Math.round(click_time - lastEventTime); // Outbound RT: time from items visible to image click
            var imgFileName = event.target.src.split('/').pop();
            
            // Store selection with outbound RT
            var selection = {
              imgId: event.target.id,
              imgSrc: imgFileName,
              position: display_locs[parseInt(event.target.id.replace('img', ''), 10)], // Save the position
              outboundRT: outboundRT,
              inboundRT: null // Will be filled later when fixation is clicked
            };
            
            selections.push(selection);
            imageClickTime = click_time; // Store click time for inbound RT calculation
        ...
        const show_search_array = () => {
            lastEventTime = performance.now(); // Record time when items become visible        
            ...
            fixation.addEventListener('click', () => {
              ...
              var click_time = performance.now();
              var inboundRT = Math.round(click_time - imageClickTime); // Inbound RT: time from image click to fixation click
              
              // Update the last selection with the inbound RT
              selections[selections.length - 1].inboundRT = inboundRT;
              ...
              if (selections.length >= trial.num_required_responses) {
                end_trial();
              } else {
                // Re-enable image clicks after fixation click
                lastEventTime = click_time; // Update last event time for next outbound RT
                enableImageClicks();
        ...
        const end_trial = () => {
            ...
            var trial_data = {
              ...
              outboundRT: JSON.stringify(selections.map(s => s.outboundRT)),
              inboundRT: JSON.stringify(selections.map(s => s.inboundRT)),
              imgIds: JSON.stringify(selections.map(s => s.imgId)),
              imgSrcs: JSON.stringify(selections.map(s => s.imgSrc)),
              positions: JSON.stringify(selections.map(s => s.position)),
              selections: JSON.stringify(selections), // Include full selections data
